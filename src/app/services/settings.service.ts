    ...
    this.subscription = this.loginService.loggedUser$.subscribe((utente: UtenteUtilities) => {
      if (utente) {
        ...
        if (!this.loggedUser) {
          ...
          const impostazioniVisualizzazioneLoggedUser = JSON.parse(
            this.loggedUser.getImpostazioniApplicazione().impostazioniVisualizzazione
          );
          const impostazioniVisualizzazioneNewUtente = JSON.parse(
            utente.getImpostazioniApplicazione().impostazioniVisualizzazione
          );
