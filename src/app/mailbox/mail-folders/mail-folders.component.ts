...
export class MailFoldersComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscriptions.push(
      this.mailListService.newTagInserted.subscribe((tag: Tag) => {
        if (tag && this.selectedNode) {
          ...
          if (selectedNodeType === PecFolderType.PEC) {
            ...
            this.selectedNode.children
              .find((pf) => pf.data.type === PecFolderType.TAG_CONTAINER)
              .children.push(this.buildTagNode(tag, tag.description, "fa fa-tag", false));
  ...
  ngOnInit() {
    this.subscriptions.push(
      this.pecService.getMyPecs().subscribe((myPecs: Pec[]) => {
        if (myPecs) {
          for (const pec of myPecs) {
            this.mailfolders.push(this.buildNode(pec));
          }
          this.selectRootNode(this.mailfolders[0], false);
          this.selectedNode = this.mailfolders[0].children.find(
            (childNode: MyTreeNode) => (childNode.data.data as Folder).type === FolderType.INBOX
          );
    ...
    this.subscriptions.push(
      this.intimusClient.command$.subscribe((command: IntimusCommand) => {
    ...
    if (pec.tagList) {
      for (const tag of pec.tagList) {
        if (tag.firstLevel && tag.visible) {
          ...
          switch (tag.name) {
            case "in_error":
              const tagNode: MyTreeNode = this.buildTagNode(
                tag,
                "Errori",
                "fas fa-exclamation-triangle smaller-icon error-icon tree-icon",
                false
              );
          ...
          const icon =
            tag.type === TagType.CUSTOM
              ? "material-icons local-offer-icon bigger-icon"
              : " material-icons-outlined local-offer-icon bigger-icon";
    ...
    if (event && event.item) {
      ...
      switch (menuItemSelected.id) {
        case "NewFolder":
          ...
          this.selectedNode.children.splice(
            this.selectedNode.children.length - 1,
            0,
            this.buildFolderNode(
              folderToInsert,
              { expandedIcon: "fa fa-folder-open smaller-icon", collapsedIcon: "fa fa-folder  smaller-icon" },
              true
            )
          );
          setTimeout(() => {
        ...
        case "DeleteFolder":
          ...
          if (folderToDelete.type === FolderType.CUSTOM) {
              ...
              .subscribe((messageNumber) => {
                if (messageNumber === 0) {
                  ...
                  this.primeMessageService.add({
                    severity: "error",
                    summary: "Errore",
                    detail: "Non puoi eliminare una cartella piena, prima svuotala!",
                    life: 3500,
                  });
                }
              });
            ...
            this.primeMessageService.add({
              severity: "error",
              summary: "Errore",
              detail: "Non puoi eliminare una cartella di sistema!",
              life: 3500,
            });
          }
          break;
  ...
  private updateFolder(folder: Folder): void {
    ...
    this.folderService.patchHttpCall(folderToSave, folder.id).subscribe(
      (f: Folder) => {
  ...
  private insertFolder(folder: Folder): void {
    ...
    this.previousSelectedNode = this.selectedNode;
    this.folderService.postHttpCall(folder).subscribe(
      (f: Folder) => {
        this.previousSelectedNode.data.data = f;
        const pecFolderList: Folder[] = (this.previousSelectedNode.parent.data.data as Pec).folderList;
  ...
  private deleteFolder(folder: Folder) {
    this.folderService.deleteHttpCall(folder.id).subscribe(
      (res) => {
        const nodeElementIndex: number = this.selectedNode.parent.children.findIndex(
          (element) => element.data.data.id === folder.id
        );
      ...
      (error) => {
        this.primeMessageService.add({
          severity: "error",
          summary: "Errore",
          detail: "Errore nell'eliminazione della cartella. Riprova tra poco e se il problema persiste contatta BabelCare.",
          life: 3500,
        });
      }
    );
  ...
  private deleteTag(tag: Tag) {
    this.tagService.deleteHttpCall(tag.id).subscribe(
      (res) => {
        const nodeElementIndex: number = this.selectedNode.parent.children.findIndex(
          (element) => element.data.data.id === tag.id
        );
        this.selectedNode.parent.children.splice(nodeElementIndex, 1);
      ...
      (error) => {
        this.primeMessageService.add({
          severity: "error",
          summary: "Errore",
          detail: "Errore nell'eliminazione del tag. Riprova tra poco e se il problema persiste contatta BabelCare.",
          life: 3500,
        });
      }
    );
