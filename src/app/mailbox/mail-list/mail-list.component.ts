...
export class MailListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChild("noteArea", {}) private noteArea: any;
  @ViewChild("idtag", {}) private inputTextTag: any;
  ...
  private timeoutOnFocusEvent: any = null;
  ...
  private VIRTUAL_ROW_HEIGHTS: any = {
    small: 79,
    medium: 84,
    big: 89,
  };
  ...
  private manageIntimusInsertCommand(command: IntimusCommand, ignoreSameUserCheck: boolean = false, times: number = 1) {
    ...
    if (
      ...
      ((params.entity === RefreshMailsParamsEntities.MESSAGE_TAG &&
        ...
        params.newRow &&
        params.newRow["id_tag"] &&
        params.newRow["id_tag"] === this.pecFolderSelected.data.id) ||
      ...
      // se non sono l'utente che ha eseguito l'azione
      ...
      if (
        ...
        setTimeout(() => {
          // il setTimeout forse non serve, ma ho paura che se lo tolgo si rompa qualcosa
   ...
   */
  private manageIntimusDeleteCommand(
    ...
    if (
      (ignoreSameUserCheck || // se l'utente che ha eseguito il comando sono io non devo fare nulla a menoche non passo ignoreSameUserCheck = true
        ...
        (params as any)["id_utente"] !== this.loggedUser.getUtente().id) && // se non ci sono nè newRow, nè oldRow allora è un caso custom di elimazione tag, in quel caso l'utente che esegue l'azione è in params
      ((params.entity === RefreshMailsParamsEntities.MESSAGE_TAG &&
        params.newRow &&
        params.oldRow &&
        ...
        (params.entity === RefreshMailsParamsEntities.MESSAGE_TAG &&
          !params.oldRow &&
          !params.newRow &&
          (params as any)["id"] === this.pecFolderSelected.data.id) ||
        (params.entity === RefreshMailsParamsEntities.MESSAGE_FOLDER &&
          params.oldRow["id_folder"] === this.pecFolderSelected.data.id) ||
      ...
      if (messageIndex >= 0 && !!!(this.messages[messageIndex] as any)["moved"]) {
        ...
        if (permanentDelete) {
          // il messaggio è stato cancellato dal cestino
          ...
          if (params.oldRow && params.oldRow["id_folder"]) {
            // non dovvrebbe mai capitare
            ...
            !params.newRow &&
            params.entity === RefreshMailsParamsEntities.MESSAGE_TAG &&
            params.operation === RefreshMailsParamsOperations.DELETE &&
            (params as any)["id"]
          ) {
            movedInfo = `al messaggio è appena stata rimossa l'etichetta ${(params as any)["tag_description"]} da ${
              (params as any)["persona"]
            }`;
          } else if (
            !params.oldRow &&
            !params.newRow &&
            ...
            // non dovvrebbe mai capitare
            movedInfo = `il messaggio è appena stato rimosso dalla cartella ${(params as any)["folder_description"]} da ${
              (params as any)["persona"]
            }`;
          }
        }
      ...
      (!params.oldRow && !params.newRow && (params as any)["id_utente"] !== this.loggedUser.getUtente().id)
    ) {
  ...
  private manageIntimusUpdateCommand(command: IntimusCommand, ignoreSameUserCheck: boolean = false) {
    ...
    if (
      params.entity === RefreshMailsParamsEntities.MESSAGE_FOLDER &&
      !!!params.oldRow["deleted"] &&
      !!params.newRow["deleted"]
    ) {
      ...
      if (params.entity === RefreshMailsParamsEntities.MESSAGE_TAG && params.oldRow["id_tag"] !== params.newRow["id_tag"]) {
        ...
        params.entity === RefreshMailsParamsEntities.MESSAGE_FOLDER &&
        params.oldRow["id_folder"] !== params.newRow["id_folder"] &&
        this.pecFolderSelected.type === PecFolderType.FOLDER
      ) {
  ...
  private reloadMessage(idMessage: number, params: RefreshMailsParams) {
    ...
    if (reload) {
      // se devo ricaricare il messaggio
  ...
  private refreshBadges(params: RefreshMailsParams) {
    ...
    if (params.entity === RefreshMailsParamsEntities.MESSAGE_TAG) {
      ...
      // per tutti gli altri cambiamenti ricarico i badge delle cartelle interessati nel comando
      if (params.newRow) {
  ...
  private refreshOtherBadgeAndDoOtherOperation(command: IntimusCommand) {
    ...
    if (params.entity === RefreshMailsParamsEntities.MESSAGE_TAG) {
      ...
      if (params.newRow && params.newRow["id_utente"] === this.loggedUser.getUtente().id) {
        ...
        if (params.oldRow) {
          // eliminazione tramite framework
  ...
  public openDetailPopup(event: any, row: any, message: any) {
    ...
    this.subscriptions.push({
      ...
      subscription: this.mailListService
        ...
        .subscribe((data) => {
          ...
          if (this.storedLazyLoadEvent && this.storedLazyLoadEvent.forceUpdate) {
            this.storedLazyLoadEvent.forceUpdate();
          }
  ...
  public buildTableEventFilters(filtersDefinition: FilterDefinition[]): {
  ...
  private getEmlSource(message: Message): string {
    ...
    if (
      message.messageFolderList &&
      message.messageFolderList[0] &&
      message.messageFolderList[0].idFolder.type === FolderType.OUTBOX
    ) {
  ...
  private setContextMenuItemLook() {
    this.cmItems.map((element) => {
      if (this.mailListService.selectedMessages.some((message: any) => !!message["moved"])) {
  ...
  private checkTagAndConfirm(queryParams: any) {
    if (queryParams && queryParams.order === 1) {
      // 1 indica che l'etichetta è applicata, quindi è da rimuovere
      this.confirmationService.confirm({
    ...
    this.mailListService.saveNoteAndUpdateTag(this.noteObject).subscribe(
      (res: BatchOperation[]) => {
        ...
        this.messagePrimeService.add({
          severity: "success",
          summary: "Successo",
          detail: "Nota salvata correttamente",
        });
      },
      (err) => {
        console.log("ERR = ", err);
  ...
  public chooseRegistrationType(event: any, registrationType: any) {
    if (!registrationType && event) {
      // vengo dal click sul menu
      if (!event.item.items) {
        if (event.item.queryParams.isPecDellAzienda === false) {
          this.confirmationService.confirm({
            header: "Conferma",
            message:
              "<b>Attenzione! Stai avviando la protocollazione su una azienda non associata alla casella selezionata su cui è arrivato il messaggio.</b><br/><br/>Sei sicuro?",
            icon: "pi pi-exclamation-triangle",
            accept: () => {
      ...
      // vengo dalla dialog di conferma
  ...
  public getRegistrationStatus(message: Message): string {
    ...
    if (registeredMessageTag) {
      ...
      if (
        Utils.arrayOverlap(
          idAziende,
          this.loggedUser.getUtente().aziendeAttive.map((azienda) => azienda.id)
        ).length > 0
      ) {
        return "REGISTERED";
      }
      ...
      switch (registrationStatus) {
        ...
        case "NOT_REGISTRABLE":
          this.messagePrimeService.add({
            severity: "warn",
            summary: "Attenzione",
            detail: "Questo messaggio non può essere protocollato.",
            life: 3500,
          });
          break;
        case "IN_REGISTRATION":
  ...
  private buildSingleRegistrationAdditionaData(additionalDataElement: any, messageTag: MessageTag): any {
    ...
    return {
      numeroProposta: additionalDataElement.idDocumento
        ? additionalDataElement.idDocumento.numeroProposta
    ...
    switch (readdressStatus) {
      ...
      case "NOT_READDRESSABLE":
        this.messagePrimeService.add({
          severity: "warn",
          summary: "Attenzione",
          detail: "Questo messaggio non può essere reindirizzato.",
          life: 3500,
        });
        break;
    }
  ...
  private buildMessageReaddres(message: any, tagName: any): string {
    let testo = "";
    const messageTag = message.messageTagList.find((mt: any) => mt.idTag.name === tagName);
    if (messageTag) {
      for (const mtAdditionalData of messageTag.additionalData) {
        if (tagName === "readdressed_in") {
          testo +=
            `<b>${new Date(messageTag.inserted).toLocaleDateString("it-IT", {
              hour: "numeric",
              minute: "numeric",
            })}</b>: ` +
  ...
  public getTaggedStatus(message: Message): any {
    ...
    const messagetagList = message.messageTagList.filter((mt) => {
  ...
  public tagsMenuStatus(status: any) {
    switch (status) {
   ...
   * @param rowData
   */
  public onRowFocus(event: any, rowData: Message) {
    //paroladacercare
    setTimeout(() => {
      this.setAccessibilityProperties(false);
  ...
  private manageMessageSelection(rowData: any) {
